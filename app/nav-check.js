...
(async()=>{
  ...
  const screens=JSON.parse(process.argv[2]);
  for(const [label,name,params] of screens){
    ...
    console.log(`${errors.length===0?'PASS':'FAIL'} [${label}] ${name} nodes=${n} :: ${body}`);
    if(errors.length) console.log('      ERR:', errors[0].slice(0,200));
  }
  process.exit(0);
})();
